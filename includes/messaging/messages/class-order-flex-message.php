...
class WLM_Order_Flex_Message {
    ...
    public static function create_test_message() {
        // 讀取測試資料 JSON 檔案
        $test_data_file = WLM_PLUGIN_DIR . 'includes/messaging/messages/test-data.json';
        ...
        // 從 JSON 取得資料
        $order_id = isset($test_data['order_id']) ? intval($test_data['order_id']) : 0;
        $order_number = isset($test_data['order_number']) ? strval($test_data['order_number']) : '';
        $payment_method_title = isset($test_data['payment_method_title']) ? strval($test_data['payment_method_title']) : '';
        
        // 如果訂單編號為空，使用預設值
        if (empty($order_number)) {
            $order_number = $order_id > 0 ? strval($order_id) : 'TEST';
        }
        
        // 如果付款方式為空，使用預設值
        if (empty($payment_method_title)) {
            $payment_method_title = '測試付款方式';
        }
        
        // 處理訂單總額：優先使用格式化後的總額，否則使用原始總額並格式化
        $order_total_raw = isset($test_data['order_total']) ? floatval($test_data['order_total']) : 0;
        $order_currency = isset($test_data['currency']) ? strval($test_data['currency']) : 'TWD';
        
        if (isset($test_data['order_total_formatted']) && !empty($test_data['order_total_formatted'])) {
            $order_total = strval($test_data['order_total_formatted']);
        } else {
            $order_total = self::format_order_total($order_total_raw, $order_currency);
        }
        ...
        // 記錄除錯資訊（只在 WP_DEBUG 啟用時記錄）
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[WLM Test Message Debug] ============================================');
            error_log('[WLM Test Message Debug] Order ID: ' . $order_id);
            error_log('[WLM Test Message Debug] Order Number: ' . $order_number);
            error_log('[WLM Test Message Debug] Payment Method: ' . $payment_method_title);
            error_log('[WLM Test Message Debug] Order Total: ' . $order_total);
            error_log('[WLM Test Message Debug] Product List Count: ' . count($product_list));
            error_log('[WLM Test Message Debug] Order View URL: ' . $order_view_url);
            error_log('[WLM Test Message Debug] Has Footer: ' . (!empty($order_view_url) ? 'Yes' : 'No'));
            error_log('[WLM Test Message Debug] Flex Message JSON:');
            error_log(json_encode($flex_message, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
            error_log('[WLM Test Message Debug] ============================================');
        }
